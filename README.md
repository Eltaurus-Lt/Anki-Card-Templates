...
With the help of the [complementary Add-on](#the-support-add-on), the template supports Memrise keyboard shortcuts, combining them with the Anki ones and adding a few shortcuts of its own, to ensure full control of the Cards on desktop without requiring a mouse. Depending on the context, `Enter` submits an answer, flips to the info screen, or advances to the next Card, automatically rating the submitted answer either "Again" or "Good" (this is a Memrise shortcut replacing Anki's default `Enter` = rate "Good"). To prevent accidental submission of empty answers, `Enter` has a small cooldown after each automatic Card flip.
