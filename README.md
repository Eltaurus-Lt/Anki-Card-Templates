...
With the help of the [complementary Add-on](#the-support-add-on), the template supports Memrise keyboard shortcuts, combining them with the Anki ones and adding a few shortcuts of its own, to ensure full control of the Cards on desktop without requiring a mouse.

Depending on the context, `Enter` submits an answer, flips to the info screen, or advances to the next Card, automatically rating the submitted answer either "Again" or "Good" (this is a Memrise shortcut replacing Anki's default `Enter` = rate "Good"). To prevent accidental submission of empty answers, `Enter` has a small cooldown after each automatic Card flip.

On the front side of a Card, number keys `1`–`0` can be used to answer multiple-choice and tapping questions (Memrise shortcuts), while on the back, the numbers `1`–`4` manually rate a Card "Again", "Hard", "Good", or "Easy" (respecively), overruling any automatically assigned rating (Anki shortcut). If a Card, by accident, is rated incorrectly, `Ctrl+Z`\*\*\* can be used to go back and redo the Card (Anki shortcut). When a correct answer is submitted, the default Memrise behaviour would be to automatically flip to the next Card, skipping the info screen. In the template, pressing `Space` before the flip shows the info screen regardless of the answer correctness (original Template shortcut replacing Anki's default `Space` = rate "Good"). 

`Tab` cycles through audio buttons on the info screen (Anki shortcut). When an audio button is selected using `Tab` (indicated by a circular outline), pressing `Enter` replays the audio (original Template shortcut), instead of invoking any of the Memrise actions described previously:
