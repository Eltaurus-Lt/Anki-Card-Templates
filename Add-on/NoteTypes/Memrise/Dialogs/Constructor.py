...
# todo: review private/public variables
class ConstructorDialog(QDialog):
    def updPresetList(self):
        self.presetList = utils.list_presets()
    ...
    def savePreset(self):
        ...
        utils.save_preset(currentPreset, self.get_preset_options())
    ...
    def loadPreset(self):
        # if modified: warning...
        preset = utils.load_preset(self.preset.currentText())
        if not preset:
            tooltip(f"error loading {self.preset.currentText()} file")            
            return
    ...
    def __init__(self):
        ...
        themeList = utils.list_themes()
        self.theme.addItems(themeList)
        self.theme.setCurrentIndex(utils.default_theme(themeList))
